 ...
 * Generates cryptographically random anti-CSRF tokens.
 *
 ...
 */
export function generateCSRFToken(params = {}) {
  const bytes = params.bytes ?? 32;
  const format = params.format ?? "base64url";

  if (bytes < 16 || bytes > 64) {
    throw new Error(`CSRF token size must be 16–64 bytes, got ${bytes}`);
  }
